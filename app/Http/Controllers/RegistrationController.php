...
$data=[];
	...
	return view("registration.registration",compact("data"));

}
//chrome://flags/#unsafely-treat-insecure-origin-as-secure
public function qr_code(Request $request){
$data=	DB::table('barcode_number')->get();
return view("registration.qr_code",compact("data"));
...
public function scanner(Request $request){
	$data=	DB::table('barcode_number')->get();
	return view("registration.scanner",compact("data"));
...
public function add(Request $request){
  ...
  // check for required fields
	 ...
	 $name= $request->post('name');
		...
		$date = date('Y-m-d H:i:s');
		 ...
		 if(!isset($name)||!isset($address)||!isset($mobile)||!isset($registration_no)||!isset($pwd)){
  ...
  try{
  DB::table('registration')->insert($insert_array);
  ...
  try{
  DB::table('registration')->where("id",$id)->update($insert_array);
...
public function get_registrations(Request $request){
	...
	if($search==""){
	try{
	$grand_data = DB::table('registration')
	...
	//$dt->delete=route('delete_registration',['id'=>$dt->id]);
	$dt->edit=route('edit_registration',['id'=>$dt->id]);
...
public function edit_registration(Request $request,$id){
	$data=DB::table("registration")
  
	->where("registration.id",$id)->get();
	
 
   return view("registration.edit",compact("data","id"));
...
public function disable(Request $request,$id,$status){
	...
	$data=DB::select('call disable_registration(?,?)',array($id,$stat));
	     ...
	     return \Redirect::route("registration.vaccination");
...
public function checkups(Request $request,$id){
	...
	return view("registration.checkups",compact("data","checkups"));
